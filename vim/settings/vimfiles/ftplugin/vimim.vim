...
" --------------------------------
    ...
    call s:vimim_initialize_four_corner()
...
if a:start
    ...
    " hunt for easter eggs only if enabled
    " ------------------------------------
    if s:vimim_easter_egg > 0
        let eggs = s:vimim_easter_egg(keyboard)
        if len(eggs) > 0
            return eggs
        endif
    endif

    " English number => Chinese number
    " --------------------------------
    if s:chinese_input_mode < 2 && a:keyboard =~? '^i'
        ...
        if len(chinese_numbers) > 0
            ...
            return s:vimim_popupmenu_list(chinese_numbers, 3)
        endif
    endif
    ...
    if s:vimim_diy_mixture_im > 0
        let keyboard_dddd = s:vimim_keyboard_number_shortcut(keyboard)
        if (len(keyboard_dddd) == 6 && keyboard_dddd =~ '^\l\l\d\{4}$')
        \|| (len(keyboard_dddd) == 5 && keyboard_dddd =~ '^\l\d\{4}$')
            let s:keyboard_sentence = ''
        endif
        let keyboards = s:vimim_diy_keyboard(keyboard_dddd)
        let results = s:vimim_diy_results(keyboards)
        if len(results) > 0
            if len(results) == 1 && s:vimim_tab_for_one_key > 0
                let s:insert_without_popup_flag = 1
            endif
            return s:vimim_popupmenu_list(sort(results), localization)
        endif
    endif
    ...
    " -----------------------------------------------------
    if match_start < 0
        let has_delimiter = match(keyboard, '[.]')
        let char_last = strpart(keyboard, len(keyboard)-1, 1)
        let results = []
        if has_delimiter < 0
            if s:vimim_diy_mixture_im > 0
            \&& (len(s:keyboard_sentence) < 1
            \|| (len(s:keyboard_sentence) > 0
            \&& s:diy_word_after_word > 0 ))
                let results = s:vimim_diy_dddd_match(lines, keyboard)
                if empty(results)
                    let results = s:vimim_diy_ldddd_match(lines, keyboard)
                endif
            endif
            if empty(results)
        ...
        elseif has_delimiter > 0 && char_last != delimiter
            let results = s:vimim_dot_by_dot_match(lines, keyboard)
        endif
        if empty(results)
            let s:keyboard_sentence = ''
        else
...
" ---------------------------------------------
    if s:vimim_fuzzy_double_pinyin < 1
    \|| len(a:keyboard) < 4
        return 0
    endif
    let sheng_mu = "[aeiouv]"
...
" -----------------------------------------------
function! s:vimim_diy_dddd_match(lines, keyboard)
" -----------------------------------------------
    let demo = '5000004000605102 zhongwenmangda'
    if empty(a:lines)
    \|| empty(a:keyboard)
    \|| len(a:keyboard)%4 != 0
    \|| a:keyboard =~ '\D'
    ...
    " -----------------------------------------------------
    let s:diy_word_after_word = 1
    let keyboard = strpart(a:keyboard, 0, 4)
    let s:keyboard_sentence = strpart(a:keyboard, 4)
    " -----------------------------------------------------
    let results = []
    if len(keyboard) > 1
        let results = s:vimim_quick_fuzzy_search(keyboard)
...
" ------------------------------------------------
function! s:vimim_diy_ldddd_match(lines, keyboard)
" ------------------------------------------------
    let demo = 'sssjjzdoloslosonfofo shanzhaishaonv'
    if empty(a:lines)
    \|| empty(a:keyboard)
    \|| len(a:keyboard)%5 != 0
    \|| strpart(a:keyboard,0,1) =~ '\d'
    \|| s:chinese_input_mode > 1
        return []
    endif
    " -----------------------------------------------
    let s:diy_word_after_word = 2
    let keyboard = strpart(a:keyboard, 0, 5)
    let s:keyboard_sentence = strpart(a:keyboard, 5)
    " -----------------------------------------------
    let results = []
    if len(keyboard) > 1
        let keyboard = s:vimim_keyboard_number_shortcut(keyboard)
...
" -----------------------------------------------
    ...
    \|| empty(a:keyboard)
    \|| len(a:keyboard) < 5
    \|| s:vimim_fuzzy_double_pinyin(a:keyboard) > 0
    \|| s:chinese_input_mode > 1
        return []
    endif
    let s:diy_word_after_word = 0
...
" -------------------------------------------------
function! s:vimim_dot_by_dot_match(lines, keyboard)
" -------------------------------------------------
    let demo = 'meet.teacher.say.hello.sssjj.zdolo.hhyy.sloso.nfofo'
    let keyboard = a:keyboard
    let len = len(keyboard)
    ...
    let has_delimiter = match(keyboard, '[.]')
    if has_delimiter > 0
        let s:keyboard_sentence = strpart(keyboard, has_delimiter+1)
        let keyboard = strpart(keyboard, 0, has_delimiter)
    else
        let s:keyboard_sentence = ''
    endif
    ...
    if match_start < 0
        ...
        let results = s:vimim_diy_results(keyboards)
        if empty(results)
            let results = s:vimim_fuzzy_match(a:lines, keyboard)
        endif
    else
        let results = s:vimim_exact_match(a:lines, keyboard, match_start)
    endif
    return results
endfunction
...
" ----------------------------------
    let key = "\<C-Y>"
    if len(s:keyboard_sentence) > 0 && s:chinese_input_mode < 2
        let s:bs = 0
        let space = '\<C-R>=g:vimim_ctrl_x_ctrl_u()\<CR>'
...
" ----------------------------------------
    let s:quantifiers = {}
    let s:quantifiers['0'] = '〇零'
    let s:quantifiers['1'] = '一壹'
    let s:quantifiers['2'] = '二贰'
    let s:quantifiers['3'] = '三叁'
    let s:quantifiers['4'] = '四肆'
    let s:quantifiers['5'] = '五伍'
    let s:quantifiers['6'] = '六陆'
    let s:quantifiers['7'] = '七柒'
    let s:quantifiers['8'] = '八捌'
    let s:quantifiers['9'] = '九玖'
...
" ----------------------------------------
    ...
    let keyboard_list = split(digit_alpha,'\ze')
    " ------------------------------------
    let chinese_number = ''
    let first_char = strpart(a:keyboard,0,1)
    for char in keyboard_list
...
" --------------------------------------------------
function! s:vimim_keyboard_number_shortcut(keyboard)
" --------------------------------------------------
    let keyboard = a:keyboard
    if s:vimim_four_corner < 1
...
" --------------------------------------
    ...
    let sheng_mu = "[aeiouv]" |" ma3 => 1 char;  pd2 => 2 chars
    if s:diy_double_input_state < 2
    \&& str2nr(digit_string) < 5
    \&& len(alpha_string) > 1
    \&& strpart(alpha_string, len(alpha_string)-1) =~ sheng_mu
