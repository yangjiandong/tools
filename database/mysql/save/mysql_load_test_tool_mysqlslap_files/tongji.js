...
{
	var unit_id     = "11613";
	var client_ip   = "58.221.37.140";
	var server_time = "1257848654";
	var verify_code = "86fe299a";
	var type        = "0";
	...
	var uv_id  = lz_get_cookie("lstat_bc_"+String(unit_id));
	var uv_new = 0;
	if(uv_id == ""){
		uv_new = 1;
		...
		lz_set_cookie("lstat_bc_"+String(unit_id), uv_id, 1);
	}

	var ss_cookie = lz_get_cookie("lstat_ss_"+String(unit_id)); 
	var ss_id = ""; 
	var ss_no = 0;
	if (ss_cookie == "") { 
		ss_no = 0;
		var rand = parseInt(Math.random() * 4000000000);
		ss_cookie = "0_"+lz_get_stm()+"_"+String(rand);
		lz_set_cookie("lstat_ss_"+String(unit_id), ss_cookie, 0); 
	} 
	else { 
		if (lz_get_stm() - lz_get_ctm(ss_cookie) > expire_time) { 
			...
			ss_no = lz_get_ss_no(ss_cookie) + 1;
			ss_id = lz_get_ss_id(ss_cookie);
			ss_cookie = String(ss_no) + "_" + lz_get_stm() + "_" + ss_id;
		}
